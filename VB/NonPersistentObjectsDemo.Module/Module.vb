...
Namespace NonPersistentObjectsDemo.Module
	...
	Public NotInheritable Partial Class NonPersistentObjectsDemoModule
		...
		Private Sub Application_ObjectSpaceCreated(ByVal sender As Object, ByVal e As ObjectSpaceCreatedEventArgs)
			Dim cos = TryCast(e.ObjectSpace, CompositeObjectSpace)
			If cos IsNot Nothing Then
				If Not (TypeOf cos.Owner Is CompositeObjectSpace) Then
					cos.PopulateAdditionalObjectSpaces(DirectCast(sender, XafApplication))
			...
			If npos IsNot Nothing Then
				...
				Dim tempVar As New NPGroupAdapter(npos)
				Dim tempVar2 As New NPFeatureAdapter(npos)
				Dim tempVar3 As New NPResourceAdapter(npos)
				Dim tempVar4 As New NPAgentAdapter(npos)
				Dim tempVar5 As New NPTechnologyAdapter(npos)
			End If
		End Sub
